...
sap.ui.define([
	...
	"CRUD_test/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function(BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("CRUD_test.controller.Detail", {
		...
		onInit: function() {
			...
			var oViewModel = new JSONModel({
				...
				lineUserItemListTitle: this.getResourceBundle().getText("detailLineUserItemTableHeading"),
				lineFuncItemListTitle: this.getResourceBundle().getText("detailLineFuncItemTableHeading"),
				lineUserGroupSelectValue: "",
			    lineUserGroupSelectPath: ""
			});
		...
		onDelete: function() {
			...
			//var assets = oViewModel.getProperty(sPath + "/Asset"),
			//funcs = oViewModel.getProperty(sPath + "/Function"),
			//users = oViewModel.getProperty(sPath + "/User");
		...
		onUserGroupAdd: function() {
			this.getModel("appView").setProperty("/addEnabled", false);
			var sObjectPath = this.getView().getElementBinding().getPath();
			this.getRouter().getTargets().display("createUserGroup", {
				mode: "create",
				objectPath: sObjectPath
			});
		},
		...
		onUserGroupSelect: function(oEvent) {
			...
			var cellValue = cells[0].mProperties.title;
			
			var oViewModel = this.getModel("detailView");
			var sObjectId = oViewModel.getProperty("/sObjectId");
			var aPath = "/UserGroupToUserSet(UserGroupId='" + cellValue + "',UserId='" + sObjectId + "')";
			oViewModel.setProperty("/lineUserGroupSelectValue", cellValue);
			oViewModel.setProperty("/lineUserGroupSelectPath", aPath);
			
		},
		
		
		onUserGroupDelete: function(oEvent) {
			
			var oViewModel = this.getModel("detailView");
			var aPath = oViewModel.getProperty("/lineUserGroupSelectPath");
			var cellValue = oViewModel.getProperty("/lineUserGroupSelectValue");
			var sSuccessMessage = this._oResourceBundle.getText("deleteSuccess", cellValue);
			
			if( aPath ) {											
				var fnMyAfterDeleted = function() {
					MessageToast.show(sSuccessMessage);
					oViewModel.setProperty("/busy", false);
					oViewModel.setProperty("/lineUserGroupSelectValue", "");
					oViewModel.setProperty("/lineUserGroupSelectPath", "");
				};
				
				this._callDelete([aPath], fnMyAfterDeleted);
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				var sObjectPath = this.getModel().createKey("UserProfileSet", oParameter);
